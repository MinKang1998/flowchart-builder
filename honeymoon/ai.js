...
const AI = (() => {
  ...
  async function extractReservation(file) {
    ...
    const instruction = [
      ...
      '      "endTime": "HH:MM 24시간. 항공편처럼 끝나는 시각(도착 시각 등)이 있으면 적으세요. 모르면 빈 문자열",',
      '      "endDate": "YYYY-MM-DD. endTime이 date와 다른 날짜(자정을 넘기는 항공편 등)에 해당하면 그 날짜를 적으세요. date와 같은 날짜면 빈 문자열",',
      ...
      '}',
      '규칙:',
      '- 항공권 왕복·경유는 각 구간을 별도 entry로. 출발 time과 도착 endTime을 함께 채워 하나의 이어진 칸으로 표시되게 하세요. 도착이 자정을 넘겨 다음날이면 endDate에 그 날짜를 적으세요.',
